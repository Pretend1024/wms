<template>
    <div class="upd-container" v-loading="loading">
...
const handleSubmit = async () => {
    ...
    if (relationList.length === 0) return ElMessage.warning('请选择关联的承运商和国家');
    ...
    loading.value = true;
    try {
        const res = await updAddressValidApi(payload);
...
const getDetail = async (id) => {
    loading.value = true;
    try {
        const res = await getAddressValidByIdApi({ id });
        if (res.success && res.data) {
            ...
            // 回显规则
            rulesList.value = (data.addressValidRuleList && data.addressValidRuleList.length > 0)
                ? data.addressValidRuleList
                : [{ fieldName: '', validateTypeId: null, validateRule: '', cnErrorMsg: '', enErrorMsg: '', sort: 1 }];

            // 回显关联关系 -> 拆解为已选承运商和国家
            if (data.addressValidRefList) {
                // 提取唯一承运商
                const uniqueCarriers = [...new Set(data.addressValidRefList.map(item => item.carrierCode))];
                selectedCarriers.value = uniqueCarriers.map(code => {
                    const found = allCarriers.value.find(c => c.value === code);
                    return found || { label: code, value: code };
                });

                // 提取唯一国家
                const uniqueCountries = [...new Set(data.addressValidRefList.map(item => item.countryCode))];
                selectedCountries.value = uniqueCountries.map(code => {
                    const found = allCountries.value.find(c => c.value === code);
                    if (found) {
                        return found;
                    }
                    return { label: code, value: code, nameCn: '' };
                });
            }
        }
    } catch (e) {
...
onMounted(async () => {
    ...
    try {
        ...
        // 加载详情
        if (props.initData && props.initData.id) {
            await getDetail(props.initData.id);
        }
    } catch (e) {
        console.error(e);
        loading.value = false;
