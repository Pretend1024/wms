...
const mainRules = {
    code: [{ required: true, message: '请输入承运商代码', trigger: 'blur' }],
...
const addLogisticsService = () => {
    ...
    props.formData.carrierServiceAddDTOList.push({ code: '', name: '', statusId: '', remark: '' });
};
...
const validateServices = () => {
    // 过滤掉 null 或 undefined 的表单引用
    const validForms = serviceFormRefs.value.filter(f => !!f);
    ...
    const promises = validForms.map(form =>
        new Promise((resolve, reject) => {
            form.validate(valid => {
                valid ? resolve() : reject(new Error('服务表单校验失败'));
            });
        })
    );
...
defineExpose({ validateMain, validateServices });
</script>
