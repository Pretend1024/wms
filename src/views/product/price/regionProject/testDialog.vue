<template>
    <el-dialog title="分区模板测试" v-model="dialogVisible" width="1000px" align-center @close="handleReset" destroy-on-close>
        <el-splitter>
            ...
            <el-splitter-panel style="flex-basis: 1000px;display: flex;flex-direction: column;">
                <el-table :data="tableData" border style="width: 100%" height="300px">
                    <el-table-column prop="regionCode" label="分区" width="120" />
...
import { getProductShipwayRegionProjectSearchApi } from '@/api/productApi/shipway.js'
// 接收外部参数
...
const handleSearch = async () => {
    formRef.value.validate(async (valid) => {
        if (!valid) return;
        const res = await getProductShipwayRegionProjectSearchApi({
            ...form.value,
            regionProjectId: props.regionProjectId
        })
