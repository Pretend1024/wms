<template>
    <div class="viewArea">
        ...
        <el-dialog v-model="detailDialogVisible" :title="dialogTitle" width="800" align-center destroy-on-close>
            <!-- 动态加载新增或编辑的表单组件 -->
            <component :is="currentForm" :id="detailId" :view="view" />
            <template #footer>
                <div class="dialog-footer">
                    <el-button @click="detailDialogVisible = false">{{ getButtonText('close') }}</el-button>
                </div>
            </template>
        </el-dialog>
        <!-- 导出弹窗 -->
...
const handleSearch = (data) => {
    ...
    // 深拷贝 data 避免修改原始对象
    // const newData = { ...data };
    // newData.orgId = newData.orgId ? newData.orgId[newData.orgId.length - 1] : '';

    // // 定义每个 view 对应的必要参数
    // const viewRequiredFields = {
    //     1: ['warehouseCodeList', 'customerCodeList', 'inOrderNoList', 'inShelfDate', 'zoneCode', 'locationCodeList', 'skuList', 'qualityId', 'statusId', 'qtyTotalMin', 'way', 'createdTimeBegin', 'createdTimeEnd'],
    //     2: ['warehouseCodeList', 'customerCodeList', 'zoneCode', 'locationCodeList', 'skuList', 'qualityId', 'way', 'createdTimeBegin', 'createdTimeEnd'],
    //     3: ['warehouseCodeList', 'customerCodeList', 'inOrderNoList', 'skuList', 'qualityId', 'way', 'createdTimeBegin', 'createdTimeEnd'],
    //     4: ['warehouseCodeList', 'customerCodeList', 'inOrderNoList', 'zoneCode', 'locationCodeList', 'skuList', 'qualityId', 'way', 'createdTimeBegin', 'createdTimeEnd'],
    //     5: ['warehouseCodeList', 'customerCodeList', 'skuList', 'qualityId', 'way', 'createdTimeBegin', 'createdTimeEnd']
    // };

    // // 获取当前 view 对应的必要参数
    // const requiredFields = viewRequiredFields[view.value] || [];
    ...
    // // 处理动态字段
    // const dynamicField = filteredData.way;
    // if (dynamicField) {
    //     filteredData[dynamicField] = newData[dynamicField];
    // }

    // initValues.value = filteredData;
...
const columns = ref([
    ...
    { label: '操作', prop: 'action', width: '170', fixed: 'right', slot: 'customBtn' }
])
...
const handleTableExpand = (row, expanded) => {
    const rowId = row.id; // 确保行数据有唯一标识id（根据实际字段调整）
    console.log('展开/收起事件：', rowId, expanded);
    if (expanded) {
        expandedRowIds.add(rowId); // 展开：添加到Set
    } else {
        expandedRowIds.delete(rowId); // 收起：从Set移除
    }
};
...
const dialogTitle = t('listDetail');
