    ...
    <div class="viewArea">
            ...
            <hydTable :footer="footer" :tableData="tableData" :columns="columns" :pagination="pagination"
                ...
                <template #table-buttons>
                    ...
                    <!-- <el-dropdown trigger="click">
                        ...
                        <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item @click="handleAudit(20)">{{ getButtonText('auditPass')
                                    }}</el-dropdown-item>
                                <el-dropdown-item @click="handleAudit(30)">{{ getButtonText('auditReject')
                                    }}</el-dropdown-item>
                            </el-dropdown-menu>
                        </template>
        </el-dropdown> -->
                ...
                <template #customBtn="{ row }">
                    <div style="display: flex;">
                        ...
                        <!-- 启用/停用 -->
                        <div class="cursor-pointer" @click="handleStatus(row)"
                            :style="{ color: row.isActive ? 'red' : 'green' }">
                            <el-icon>
                                <Open />
                            </el-icon>
                            <span>{{ row.isActive ? getButtonText('disable') : getButtonText('enable') }}</span>
                        </div>
                        <!-- 详情 -->
                ...
                <template #isActive="{ row }">
                    <span :style="{ color: row.isActive ? 'green' : 'red' }">{{ row.isActive ? '是' : '否 ' }}</span>
                </template>
            </hydTable>
        </div>
        ...
        <el-dialog v-model="centerDialogVisible" :title="dialogTitle" width="750" align-center destroy-on-close>
            <component :is="currentForm" ref="childFormRef" :initData="dialogMode === 'add' ? {} : editInitData"
                ...
                :nationOptions="nationOptions" :sceneOptions="sceneOptions" />
            <template #footer>
...
const formConfig = ref([
    {
        type: 'select', label: '状态', prop: 'isActive', options: [
            { value: 1, label: '启用' },
            { value: 0, label: '停用' }
        ]
    },
    { type: 'select', label: '授信状态', prop: 'sceneId', options: [] },
...
const columns = ref([
    ...
    { label: '适用场景', prop: 'sceneName', width: '130', sortable: true },
    { label: '服务类型描述', prop: 'description', width: '200' },
    { label: '是否启用', prop: 'isActive', width: '110', sortable: true, slot: 'isActive' },
    { label: '默认单位', prop: 'defaultUnit', width: '120' },
    { label: '币种', prop: 'currency', width: '100', sortable: true },
    { label: '默认单价', prop: 'defaultPrice', width: '120' },
    { label: '序号', prop: 'sortNo', width: '85', sortable: true },
...
const dialogTitle = computed(() => {
    if (dialogMode.value === 'add') {
        return t(`vas_vas_vasServiceType_list.addTitle`);
    } else if (dialogMode.value === 'upd') {
        return t(`vas_vas_vasServiceType_list.updTitle`);
    } else { // info模式
        return t(`vas_vas_vasServiceType_list.infoTitle`); // 需确保i18n中有infoTitle配置
    }
});
...
const handleDialogConfirm = async () => {
    ...
    try {
        ...
        let res;
        if (formData.id) {
            res = await updVasServiceTypeByIdApi(formData); // 编辑接口
        } else {
            res = await addVasServiceTypeApi(formData); // 新增接口
        }
...
const handleStatus = async (row) => {
    if (row.isActive) {
        ElMessageBox.confirm(
            ...
            const res = await setEnableDisableApi({ id: row.id, isEnable: !row.isActive });
    ...
    ElMessageBox.confirm(
        ...
        .then(async () => {
            ...
            for (let i = 0; i < selectionRows.value.length; i++) {
                const res = await delVasServiceTypeByIdApi({ id: selectionRows.value[i].id });
                delData.value.push({
...
const getList = async (currentPage, pageSize, orderByStr) => {
    try {
        const res = await getVasServiceTypePageApi({
            page: currentPage,
            pageSize: pageSize,
            orderBy: orderByStr,
...
onMounted(async () => {
    ...
    sceneOptions.value = sceneRes.data.map(item => ({
        value: item.id,
        label: item.name
    }))
    formConfig.value[1].options = sceneOptions.value;
})
