        ...
        <div class="tableDiv">
            <hydTable :tableData="tableData" :columns="columns" :pagination="pagination" :enableSelection="true"
                ...
                <template #table-buttons>
                    <el-button type="primary" @click="handleAdd" v-permission="'position:add'" :icon="Plus">
                        {{ getButtonText('add') }}
                    </el-button>
                ...
                <template #isInit="{ row }">
                    <span :style="{ color: row.isInit ? 'green' : 'red' }">{{ row.isInit ? '是' : '否' }}</span>
                </template>
            </hydTable>
        </div>
        <!-- 弹窗 -->
...
import { getPositionListApi, addPositionDataApi, updatePositionDataApi, delPositionDataApi } from "@/api/baseApi/org.js";
...
const dialogTitle = computed(() => t(`base_org_position_list.${dialogMode.value}Title`)); // 可根据模式调整标题
