<template>
    <div class="viewArea">
        ...
        <batchOperationn :isVisible="delDialogVisible" :tableData="delData" :nameField="'id'"
            :nameLabel="t('base_wh_locationSizeType_list.nameLabel')" @close="delColse"
            :promptMessage="promptMessage" />
    </div>
</template>

<script setup name='库位类型'>
/* 1. 引入 */
// 1.1 Vue核心及插件
...
// 1.3 API引入
import {
    getWhLocationSizeTypeListApi,
    addWhLocationSizeTypeApi,
    updWhLocationSizeTypeApi,
    delWhLocationSizeTypeApi
...
const columns = ref([
    ...
    { label: '长', prop: 'length', width: '150', sortable: true },
    { label: '宽', prop: 'width', width: '150', sortable: true },
    { label: '高', prop: 'height', width: '150', sortable: true },
    { label: '体积(M³)', prop: 'volume', width: '150', sortable: true },
...
// 弹窗标题
const dialogTitle = computed(() => t(`base_wh_locationSizeType_list.${dialogMode.value}Title`));
// 当前表单组件
...
const getList = async (currentPage, pageSize, orderBy) => {
    const res = await getWhLocationSizeTypeListApi({
        page: currentPage,
        pageSize: pageSize,
        orderBy
    });
...
const handleDel = () => {
    if (selectionRows.value.length === 0) {
        ElMessage({
            type: 'warning',
            message: t('base_wh_locationSizeType_list.selectDelete')
        });
        return;
    }
    ElMessageBox.confirm(
        t('base_wh_locationSizeType_list.confirmDelete', { count: selectionRows.value.length }),
        t('base_wh_locationSizeType_list.reminder'),
        {
        ...
        .then(async () => {
            ...
            delData.value = [];
            promptMessage.value = t('base_wh_locationSizeType_list.processing');
            for (let i = 0; i < selectionRows.value.length; i++) {
                const res = await delWhLocationSizeTypeApi({ id: selectionRows.value[i].id });
                delData.value.push({
            ...
            promptMessage.value = t('base_wh_locationSizeType_list.operationComplete');
        })
        .catch(() => {
            ElMessage({
                type: 'info',
                message: t('base_wh_locationSizeType_list.deleteCanceled')
            });
        });
};
...
const handleDialogConfirm = async () => {
    ...
    try {
        ...
        // 数值转换
        if (addData.value.sortNo) {
            addData.value.sortNo = Number(addData.value.sortNo);
        }
        loading.value = true;
        let res;
        if (addData.value.id) {
            res = await updWhLocationSizeTypeApi(addData.value);
        } else {
            res = await addWhLocationSizeTypeApi(addData.value);
        }
        ...
        if (res.success) {
            ...
            loading.value = false;
            // 错误提示使用 ElMessageBox
            ElMessageBox.alert(res.msg, t('base_wh_locationSizeType_list.reminder'), {
                confirmButtonText: getButtonText('confirm'),
                type: 'warning'
            });
        }
