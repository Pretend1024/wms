        ...
        <div class="tableDiv">
            <hydTable :tableData="tableData" :columns="columns" :pagination="pagination" :enableSelection="true"
                ...
                <template #statusName="{ row }">
                    <el-tag :type="row.statusId == 3 ? 'success' : 'danger'">{{ row.statusName }}</el-tag>
                </template>
                <template #async="{ row }">
...
import { getExportJobListApi, getExportJobTypeEnumApi, getExportJobStatusEnumApi, exportJobCloseByIdApi } from '@/api/sysApi/setting.js'
...
const columns = ref([
    { label: '导出编号', prop: 'jobNo', width: '200', sortable: true, fixed: 'left' },
    { label: '导出类型', prop: 'typeName', width: '160', sortable: true, fixed: 'left' },
    { label: '模板名称', prop: 'templateName', width: '175', sortable: true, fixed: 'left' },
    { label: '创建时间', prop: 'createdTime', width: '200', sortable: true },
    { label: '状态', prop: 'statusName', width: '120', sortable: true, slot: 'statusName' },
    { label: '导出总数', prop: 'exportTotal', width: '165', sortable: true },
    { label: '已导出数', prop: 'exportAlready', width: '200', sortable: true },
    { label: '进度', prop: 'progress', width: '105' },
    { label: '导出参数', prop: 'exportParam', width: '300' },
    { label: '客户名称', prop: 'customerName', width: '175' },
    { label: '文件大小', prop: 'fileSize', width: '130', sortable: true },
    ...
    { label: '耗时(s)', prop: 'elapsedTime', width: '155' },
...
const dialogTitle = computed(() => t(`sys_job_exportJob_list.${dialogMode.value}Title`)); // 可根据模式调整标题
...
onMounted(async () => {
    const typeEnumRes = await getExportJobTypeEnumApi()
    typeEnumOptions.value = typeEnumRes.data.map(item => ({ value: item.id, label: item.name }));

    const statusEnumRes = await getExportJobStatusEnumApi()
