                ...
                <el-row>
                    ...
                    <el-col :span="12">
                        <el-form-item label="计划到仓时间:" prop="plannedArrivalTime">
                            <el-date-picker v-model="formData.plannedArrivalTime" type="datetime" placeholder="选择计划到仓时间"
            ...
            <el-row>
                <el-col :span="24">
                    <inOrder-table ref="tableRef" :columns="columns" :data="tableData" index-label="箱规序号"
                        ...
                        <template #boxQty="{ row }">
                            <el-input v-model="row.boxQty" v-intNumber @blur="onBoxQtyBlur(row)" />
                        </template>
                        <template #skus="{ row }">
                            <el-form-item>
...
<script setup name="编辑入库单">
import { getCustomerLikeQueryApi, getSkuSkuDataBySkuApi } from '@/api/baseApi/sku.js'
import { getWhWarehouseApi } from '@/api/baseApi/wh.js'
import { getOrderInOrderInfoVOByIdApi, getInstockInOrderFirstLegEnumApi, getInstockInOrderTypeEnumApi, getInstockInOrderBusinessEnumApi, getInstockInOrderCabinetTypeEnumApi, updOrderInOrderApi, getInstockInOrderArrivalTypeEnumApi } from '@/api/instockApi/order.js';
...
const handleSave = async () => {
    ...
    await formRef.value.validate(async (valid) => {
        if (valid) {
            exportData()
            const res = await updOrderInOrderApi(formData.value);
