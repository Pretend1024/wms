    ...
    <el-table :data="localData" border stripe :height="'100%'" style="width: 100%" :span-method="spanMethod"
        ...
        <el-table-column :label="indexLabel" width="65" fixed="left">
            <template #default="{ row, $index }">
                <div v-if="isGroupStart(row)" class="group-start-cell">
                    ...
                    <div v-if="controlsVisible" class="btn-group">
                        <span class="btn" @click="row.id ? null : removeGroup($index)"
                            :style="{ color: row.id ? 'gray' : 'inherit', cursor: row.id ? 'not-allowed' : 'pointer' }">
                            －
                        </span>
                        <span class="btn" @click="addGroup($index)">＋</span>
                    </div>
                </div>
            </template>
        </el-table-column>
        ...
        <el-table-column v-if="hideOperationVisible ? !hideOperationVisible : controlsVisible" label="操作" fixed="right"
            ...
            <template #default="{ row }">
                <el-button icon="Plus" @click="addInGroup(getGroupKey(row))" size="small" />
                <el-button icon="Minus" @click="removeInGroup(getGroupKey(row))" size="small" />
            </template>
        </el-table-column>
    </el-table>
</template>
...
function addGroup(index) {
    const newRow = emptyRow({ _groupId: 'group_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6) });
    localData.splice(index + countInGroup(localData[index]), 0, newRow);
    updateData();
}
...
function removeGroup(index) {
    ...
    for (let i = localData.length - 1; i >= 0; i--) {
        if (getGroupKey(localData[i]) === key) {
...
// 在组内添加行
function addInGroup(groupKey) {
    const groupRows = localData.filter(r => getGroupKey(r) === groupKey);
    const lastIndex = localData.indexOf(groupRows[groupRows.length - 1]);
    const newRow = emptyRow({ _groupId: groupKey }); // 继承组 ID
    localData.splice(lastIndex + 1, 0, newRow);
    updateData();
}
// 在组内删除行
function removeInGroup(groupKey) {
    const groupRows = localData.filter(r => getGroupKey(r) === groupKey);
    if (groupRows.length <= 1) return;
    const lastIndex = localData.indexOf(groupRows[groupRows.length - 1]);
    localData.splice(lastIndex, 1);
    updateData();
}
