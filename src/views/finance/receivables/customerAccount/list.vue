...
const dialogTitle = computed(() => {
    if (dialogMode.value === 'add') {
        return t(`finance_account_customerAccount_list.addTitle`);
    } else if (dialogMode.value === 'upd') {
        return t(`finance_account_customerAccount_list.updTitle`);
    } else { // info模式
        return t(`finance_account_customerAccount_list.infoTitle`); // 需确保i18n中有infoTitle配置
    }
});
...
const handleDialogConfirm = async () => {
    ...
    try {
        ...
        if (formData.id) {
            ...
            res = await addCustomerAccountApi(formData); // 新增接口
        }
...
const getList = async (currentPage, pageSize, orderByStr) => {
    try {
        const res = await getCustomerAccountPageApi({
            page: currentPage,
            pageSize: pageSize,
            orderBy: orderByStr,
