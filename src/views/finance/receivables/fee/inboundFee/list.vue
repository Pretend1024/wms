    ...
    <div class="viewArea">
                    ...
                    <el-col>
                        <el-form-item :label="getLabel('inOrderNo')">
                            <canonicalInput v-model:listName="formData.inOrderNoList"
                                :placeholder="getPlaceholder('inOrderNo')" clearable>
                            </canonicalInput>
                        </el-form-item>
                    </el-col>
                    <el-col>
        ...
        <div class="tableDiv">
            <hydTable :footer="footer" :tableData="tableData" :columns="columns" :pagination="pagination"
                ...
                <template #inOrderCreatedTime="{ row }">
        ...
        <!-- <el-dialog v-model="joinBillVisible" title="加入账单" width="500px" align-center>
            <el-form label-width="100px">
                <el-form-item label="加入范围:">
                    <el-radio-group v-model="joinBillForm.scope">
                        <el-radio label="selection">按勾选 ({{ selectionRows.length }}条)</el-radio>
                        <el-radio label="condition">按当前查询条件</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="加入方式:">
                    <el-radio-group v-model="joinBillForm.method"
                ...
                <el-form-item label="账单号:" v-if="joinBillForm.method === 1">
                    <el-input v-model="joinBillForm.targetBillNo" placeholder="请输入账单号" />
                </el-form-item>
            </el-form>
            <template #footer>
                <el-button @click="joinBillVisible = false">取消</el-button>
                <el-button type="primary" @click="handleJoinBillConfirm" :loading="joinBillLoading">确定</el-button>
            </template>
        </el-dialog> -->
        ...
        <exportDialog ref="exportDialogRef" :selectionRows="selectionRows" :initValues="initValues" :exportType="606">
        </exportDialog>
...
import {
    getInOrderFeePageApi,
    addInOrderFeeApi,
    updInOrderFeeByIdApi,
    delInOrderFeeByIdApi,
    joinBillApi
...
const columns = ref([
    ...
    { label: '入库单号', prop: 'inOrderNo', width: '160', sortable: true, fixed: 'left' },
    { label: '入库日期', prop: 'inOrderCreatedTime', width: '200', slot: 'inOrderCreatedTime' },
...
const getList = async (page, pageSize, orderByStr = orderBy.value) => {
    ...
    try {
        ...
        const res = await getInOrderFeePageApi({
            page,
            pageSize,
            orderBy: orderByStr,
            ...params
...
const dialogTitle = computed(() => dialogMode.value === 'add' ? '新增入库费用' : '编辑入库费用');
...
const handleDialogConfirm = async () => {
    ...
    try {
        ...
        if (dialogMode.value === 'add') {
            res = await addInOrderFeeApi(formData);
        } else {
            res = await updInOrderFeeByIdApi({
                id: formData.id,
...
const handleDel = () => {
    ...
    ElMessageBox.confirm(`是否要删除${selectionRows.value.length}条数据?`, '提醒', {
        ...
        for (const row of selectionRows.value) {
            const res = await delInOrderFeeByIdApi({ id: row.id });
            resultData.value.push({
                id: `${row.inOrderNo} - ${row.feeName}`,
                msg: res.msg,
                success: res.success
            });
        }
...
const handleImport = () => {
    router.push({
        name: '导入文件',
        params: { typeId: 606, typeName: '入库单应收费用' },
    });
};
