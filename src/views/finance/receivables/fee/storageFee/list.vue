        ...
        <div class="tableDiv">
            <hydTable :footer="footer" :tableData="tableData" :columns="columns" :pagination="pagination"
                ...
                <template #table-buttons>
                    <!-- <el-button type="primary" @click="handleAdd" v-permission="'receivableFree:add'" :icon="Plus">{{ getButtonText('add') }}</el-button> -->
...
const initValues = ref({
    orgId: [], warehouseCode: '', customerCodeList: [], feeSubTypeId: '', statusId: '', createWay: '', orderNoList: [], billNoList: [], feeMainTypeId: 3
});
...
// 重置逻辑：保留 feeMainTypeId = 3
const handleReset = (data) => {
    selectDateData.value = { dateType: 10, dateRange: getDefaultDateRange() };
    initValues.value = { ...data, orgId: '', dateType: 10, dateRange: getDefaultDateRange(), feeMainTypeId: 3 };
...
// 初始化：获取仓租大类(3)下的小类
onMounted(async () => { customerOptions.value = props.initialCustomerOptions; const feeTypeRes = await getFeeTypeEnumApi({ mainTypeId: 3 }); feeTypeOptions.value = feeTypeRes.data.map(i => ({ label: i.name, value: i.id })); });
</script>
