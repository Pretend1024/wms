                    ...
                    <el-col>
                        <el-form-item :label="getLabel('feeSubTypeId')">
                            <el-select v-model="formData.feeSubTypeId" :placeholder="getPlaceholder('feeSubTypeId')"
                                clearable :disabled="!formData.feeBizTypeId">
...
import { ref, onMounted, nextTick, shallowRef, toRefs, watch } from 'vue';
...
const handleDialogConfirm = async (formData) => {
    ...
    try {
        ...
        if (dialogMode.value === 'add') {
            ...
            res = await updFeeByIdApi({ id: formData.id, confirmFeeAmount: formData.confirmFeeAmount, remark: formData.remark });
        }
