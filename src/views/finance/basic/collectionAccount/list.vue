    ...
    <div class="viewArea">
                ...
                <template #customBtn="{ row }">
                    <div style="display: flex;">
                        ...
                        <div class="cursor-pointer" @click="handleStatus(row)"
                            :style="{ color: row.statusId === 10 ? 'red' : 'green' }">
                            <el-icon>
                                <Open />
                            </el-icon>
                            <span>{{ row.statusId === 10 ? getButtonText('disable') : getButtonText('enable') }}</span>
                        </div>
                        <!-- 详情 -->
        ...
        <el-dialog v-model="centerDialogVisible" :title="dialogTitle" width="900" align-center destroy-on-close>
            <component :is="currentForm" ref="childFormRef" :initData="dialogMode === 'add' ? {} : editInitData"
                :companyOptions="companyOptions" :accountStatusEnum="accountStatusEnum"
                :paymentMethodEnum="paymentMethodEnum" :isDisabled="dialogMode === 'info'" />
            <template #footer>
...
<script setup name="收款账户">
import { ref, computed, onMounted, nextTick } from 'vue';
import { Plus, Delete, EditPen } from '@element-plus/icons-vue';
import { ElLoading, ElMessage, ElMessageBox } from 'element-plus';

// 接口导入
import { getAccountPageApi, getPaymentMethodEnumApi, getAccountStatusEnumApi, addAccountApi, updAccountByIdApi, updAccountStatusApi } from '@/api/financeApi/basic.js';
...
const formConfig = ref([
    { type: 'select', label: '支付方式', prop: 'paywayId', options: [] },
    { type: 'select', label: '账户状态', prop: 'statusId', options: [] },
    { type: 'input', label: '账号', prop: 'accountNoLike' },
...
const columns = ref([
    ...
    { label: '账户账号', prop: 'accountNo', width: '120' },
    { label: '持有人', prop: 'accountHolderName', width: '120' },
    { label: '币种', prop: 'currency', width: '120', sortable: true },
    { label: '账户状态', prop: 'statusId', width: '120', sortable: true, slot: 'statusName' },
...
const dialogTitle = computed(() => {
    if (dialogMode.value === 'add') {
        return t(`finance_account_collectionAccount_list.addTitle`);
    } else if (dialogMode.value === 'upd') {
        return t(`finance_account_collectionAccount_list.updTitle`);
    } else { // info模式
        return t(`finance_account_collectionAccount_list.infoTitle`); // 需确保i18n中有infoTitle配置
    }
});
...
const handleDialogConfirm = async () => {
    ...
    try {
        ...
        let res;
        if (formData.id) {
            res = await updAccountByIdApi(formData); // 编辑接口
        } else {
            res = await addAccountApi(formData); // 新增接口
        }
...
const handleStatus = async (row) => {
    if (row.statusId === 10) {
        ElMessageBox.confirm(
            ...
            const res = await updAccountStatusApi({ id: row.id, statusId: 20 });
        ...
        ElMessageBox.confirm(
            ...
            const res = await updAccountStatusApi({ id: row.id, statusId: 10 });
            smartAlert(res?.msg || '操作异常', res?.success || false, 1000);
            if (res?.success) {
                getList(pagination.value.currentPage, pagination.value.pageSize, orderBy.value); // 刷新列表
            }
...
const handleDel = () => {
    ...
    ElMessageBox.confirm(
        ...
        for (const row of selectionRows.value) {
            const res = await delCustomerApiApi({ id: row.id });
            delData.value.push({
                id: row.customerName || row.id, // 显示客户名或ID
                msg: res?.msg || '删除成功',
                success: res.success
            });
        }
...
onMounted(async () => {
    ...
    // 3. 加载账户状态枚举（填充搜索表单）
    const statusRes = await getAccountStatusEnumApi();
